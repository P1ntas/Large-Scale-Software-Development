...
Please be aware of the naming schema to follow this pattern and DO NOT create default branches (e.g. via GitHub), and make sure the description is well documented to understand the changes made. 

Patterns for branches will follow essentially the same guidelines as commits.

For each sprint, create a new branch from the dev branch, associated with the current sprint development you are going to do. If there is a need to change the factsheets or the docs folders, create a new branch from the documentation. If you change something on your development and you need to document the changes on the markdown files, DO NOT update it on the dev branch, to ease with conflicts.
- build/{name}
- docs/{name}
- feat/{name}
- fix/{name}
...
At the end of each sprint, the branches associated with dev and documentation will be merged into main, so the changes will appear on the deployment server.

Note: After merging a branch, please be conscious to *delete it*

Update: Each sprint, take into account that new branches that are created on top of dev should only be merged in the end (otherwhise, it would be the same as pushing directly into master). For each issue/task, pull requests should be done on top of the correspondent team development branch.
Example: branch associated with the issue #1 should be merged on top of the teamX development branch, and only in the end, the teamX development branch is merged into dev 

At the moment of closing a Pull Request, add enough information so that reviewers and other users may see the changes done. Feel free to add text or even print screens if it seems deemed. 

Remember that the reviews themselves shouldn't be done by the work group and verify if the branch it is merging into is the correct one.
